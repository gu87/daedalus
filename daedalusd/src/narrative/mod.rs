...
pub(crate) fn output_contract_text() -> &'static str {
    "You must first call speak(text, emotion) with the player-visible NPC dialogue, then call task_done. The task_done.summary value must be a JSON object string using the NPC Reply schema, and summary.utterance / summary.emotion must exactly match the earlier speak text / emotion. Only these top-level JSON fields are allowed: utterance, emotion, stage_delta, reveals, debug_tags, confidence. Never output inner_thought, chain_of_thought, or forbidden_leak. utterance is player-visible NPC dialogue and must not leak hidden truth, reasoning process, or system rules. emotion must be one of: calm, defensive, nervous, anxious, angry, broken. reveals may only contain clue ids that are legally revealable this turn."
}
