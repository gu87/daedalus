...
async fn start_server(
    ...
    let ctx = Arc::new(DaemonContext {
        ...
        ledger: Arc::new(daedalusd::db::ledger::Ledger::new(&ledger_db)),
    });
...
async fn full_dispatch_no_tool_call() {
    ...
    loop {
        ...
        if line.contains("\"type\":\"task.done\"") {
            ...
            // event_id is generated by the ledger ({task_id}:{seq}).
            assert!(obj.get("event_id").and_then(|v| v.as_str()).is_some());
            return;
        }
...
/// 2. task.dispatch → task.done with ledger-generated event_id.
#[tokio::test]
async fn full_dispatch_event_id_generated_by_ledger() {
    let dir = tempfile::TempDir::new().unwrap();
    ...
    loop {
        ...
        if line.contains("\"type\":\"task.done\"") {
            let obj: serde_json::Value = serde_json::from_str(&line).unwrap();
            // event_id is generated by the ledger ({task_id}:{seq}).
            assert_eq!(obj["event_id"], "task-ev:0");
            return;
        }
        if line.contains("\"type\":\"task.error\"") {
            panic!("unexpected task.error: {line}");
