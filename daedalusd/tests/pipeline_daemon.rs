...
impl LLMProvider for TextProvider {
    async fn chat(
        ...
        if let Some(set) = self.chunks.first() {
            for c in set {
                if let Ok(StreamChunk::Text { content: txt }) = c {
...
impl AgentLoopFactory for PipeTestFactory {
    fn build(
        &self,
        agent_id: String,
        perm: Arc<dyn PermissionBroker>,
        cancel: CancellationToken,
    ) -> Result<AgentLoop, DaedalusError> {
        let mut r = ToolRegistry::new();
        for t in &self.tools {
            r.register(Arc::clone(t)).unwrap();
        }
...
fn read_task_status(db_path: &std::path::Path, task_id: &str) -> Option<TaskStatus> {
    ...
    pipeline_db::get_task(&conn, task_id)
        .unwrap()
        .map(|r| r.status)
}
...
async fn dispatch_success_eventually_waiting_for_verification() {
    ...
    let provider = Arc::new(TextProvider {
        chunks: vec![vec![Ok(StreamChunk::Text {
            content: "ok".into(),
        })]],
    });
    let factory = Arc::new(PipeTestFactory {
        provider,
        tools: vec![Arc::new(daedalusd::tools::task_done::TaskDoneTool)],
        config: cfg.clone(),
    });
    ...
    assert_eq!(
        status,
        Some(TaskStatus::WaitingForVerification),
...
async fn task_error_hardstop_transitions_to_failed() {
    ...
    let provider = Arc::new(TextProvider {
        chunks: vec![vec![Ok(StreamChunk::ToolCall {
            id: "t1".into(),
            name: "boom".into(),
    ...
    impl daedalusd::tools::Tool for BoomTool {
        ...
        fn needs_permission(&self, _: &serde_json::Value) -> bool {
            false
        }
        fn validate(
            &self,
            _: &serde_json::Value,
            _: &daedalusd::tools::ToolContext,
        ) -> Result<(), String> {
            Ok(())
        }
        async fn execute(
    ...
    let msg = tokio::time::timeout(Duration::from_secs(5), rx.recv())
        ...
        .unwrap();
    assert!(
        matches!(msg, Message::TaskError(_)),
        "expected TaskError, got {msg:?}"
    );
    tokio::time::sleep(Duration::from_millis(200)).await;
    let status = read_task_status(&db_path, "pipe-test-task");
...
async fn orphan_scanner_updates_tasks_to_failed() {
    ...
    let status = pipeline_db::get_task(&conn, "orphan-task")
        .unwrap()
        .unwrap()
        .status;
...
async fn switch_agent_transitions_via_blocked() {
    ...
    let provider: Arc<dyn LLMProvider> = Arc::new(CounterProvider {
    ...
    let ctx = Arc::new(DaemonContext {
        ...
        db_path: db_path.clone(),
        factory,
        gate_router: Arc::new(GateRouter::new(registry, 5)),
        ledger: Arc::new(daedalusd::db::ledger::Ledger::new(&db_path)),
    });
    let td = make_td(dummy_task_card());
    let (tx, mut rx) = mpsc::channel(8);
